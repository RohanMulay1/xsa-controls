...
CLAIMS = [
    ...
    ("PILOT the random arm's paired mean delta vs baseline (5e7, superseded)",
     0.001190, "nats", "paired_tests_s_pilot_5e7.csv", "run_factorial.py --size S",
    ...
    ("the random arm's p value (underpowered 5e7 pilot, not the primary endpoint)",
     0.042, "", "paired_tests_s_pilot_5e7.csv", "run_factorial.py --size S",
     paired_field("random", "p", "paired_tests_s_pilot_5e7.csv")),
    ("the xsa arm's paired mean delta vs baseline (underpowered 5e7 pilot, not the primary endpoint)",
     0.001515, "nats", "paired_tests_s_pilot_5e7.csv", "run_factorial.py --size S",
     paired_field("xsa", "mean_delta", "paired_tests_s_pilot_5e7.csv")),
    ("the xsa arm is not significant (underpowered 5e7 pilot, not the primary endpoint)",
     0.387, "", "paired_tests_s_pilot_5e7.csv", "run_factorial.py --size S",
     paired_field("xsa", "p", "paired_tests_s_pilot_5e7.csv")),
    ...
    ("PRIMARY xsa improves validation loss against baseline at 3.999e8 "
     "tokens per run",
     -0.002924, "nats", "paired_tests_s.csv", "run_factorial.py --size S",
     paired_field("xsa", "mean_delta")),
    ("PRIMARY that improvement is significant after Holm correction",
     0.0022, "", "paired_tests_s.csv", "run_factorial.py --size S",
     paired_field("xsa", "p_holm")),
    ("PRIMARY the pre-registered primary endpoint, a matched arbitrary "
     "direction, does not improve on baseline",
     0.001056, "nats", "paired_tests_s.csv", "run_factorial.py --size S",
     paired_field("random", "mean_delta")),
    ("PRIMARY and is not significant",
     0.1969, "", "paired_tests_s.csv", "run_factorial.py --size S",
     paired_field("random", "p")),
    ("PRIMARY realised MDE for the xsa arm, computed from its paired sd",
     0.001817, "nats", "paired_tests_s.csv",
     "2.9 * sd_paired / sqrt(8), run_factorial.py",
     lambda: (lambda r: 2.9 * float(r["sd_paired"]) / 8 ** 0.5)(
         [x for x in (rows("paired_tests_s.csv") or [])
          if x["arm"] == "xsa"][0]) if rows("paired_tests_s.csv") else None),
]
