...
\begin{abstract}
Transformer architectural modifications frequently measure a statistic on internal representations, observe that it is large, remove it architecturally, and report an improvement. Interpreting that sequence requires evaluating whether the statistic exceeds its noise floor, whether it survives a geometric null, and whether the training gain requires the specific direction removed. We formulate these requirements as three reusable checks and apply them to self-attention removal, which deletes the component of an attention head's output aligned with its own value vector. Across twelve pretrained models (6,784 head rows, 124M to 6.9B parameters), an earlier-position causal null accounts for 48\% to 63\% of the head-value cosine alignment, with no monotone scale trend. Under grouped-query attention, this alignment is specific to each head's own key--value group and reverses across groups (+0.24 to +0.27 within-group, $-0.11$ to $-0.19$ across-group). Split-half reliability demonstrates that the per-head intervention effect is resolvable in GPT-2 ($r_\Delta = 0.80$) but attenuated in Pythia-160M ($0.42$) and Pythia-410M ($0.53$), whereas structural self-cosine reliability is near-perfect in all three ($r_{\text{stat}} > 0.99$); which statistic predicts its own intervention is model-dependent. In a pre-registered 3-arm by 8-seed paired factorial at $4.0\times10^{8}$ tokens per run ($399\,900\,672$ tokens), a matched arbitrary rank-one direction does not differ from baseline ($+0.0011$ nats, $p = 0.197$), while self-directed removal lowers validation loss ($-0.0029$ nats, Holm-corrected $p = 0.0023$) and outperforms the matched arbitrary control directly ($-0.0040$ nats, $p = 0.0018$). The training benefit is specific to the direction removed at 50.9M scale, but a large cosine alignment alone does not establish redundancy in frozen models.
\end{abstract}
...
We evaluate these questions through three sequential checks:

\paragraph{Check 0: is the effect resolvable?} Before interpreting correlations
between representation statistics and intervention outcomes, we estimate the
split-half reliability $r_\Delta$ of the behavioural effect and $r_{\text{stat}}$
of the structural statistic. Their geometric mean $\sqrt{r_\Delta r_{\text{stat}}}$
provides an attenuation diagnostic. We classify an effect as reliable at
$r_\Delta \ge 0.6$, attenuated at $0.3 \le r_\Delta < 0.6$, and unresolvable below
$0.3$; this classification relies on $r_\Delta$ directly rather than the geometric mean.

\paragraph{Check 1: does alignment exceed background geometry?} We compare
$\cos(\yv_i, \mathbf{v}_i)$ against the cosine computed with a null partner drawn
uniformly from earlier positions in the same sequence. The difference measures
excess alignment beyond shared causal representation geometry.

\paragraph{Check 2: does the training benefit depend on direction?} We train
self-directed and arbitrary-direction arms with identical gate parameterisations
and evaluate each against baseline and against each other across paired random seeds.
The protocol pre-registers the arbitrary-direction comparison as primary.
...
directly rather than assuming a uniform anisotropy across the ladder. Across
the multi-head ladder, the earlier-position null accounts for $48.1\%$ to
$63.1\%$ of $\cos(\yv_i, \mathbf{v}_i)$. Even at $6.9$B parameters, $58.1\%$
of the alignment ($0.198$ of $0.340$) remains attributable to the null,
showing that background geometry persists at billion-parameter scales rather
than vanishing with model capacity.
...
Under the Check~0 thresholds, the behavioural effect is reliable in GPT-2
($r_\Delta = 0.795$) and attenuated in Pythia-160M ($0.419$) and Pythia-410M
($0.531$; Table~\ref{tab:reliability}). All three models avoid the unresolvable
category ($r_\Delta < 0.3$). Structural self-cosine reliability is near-perfect
across all three models ($0.991$ to $0.997$). The attenuation is therefore driven
by measurement noise in the single-head intervention effect rather than the
underlying representation statistic. For Pythia-160M, dividing the raw self-cosine
correlation of $0.149$ by the attenuation diagnostic $0.645$ yields $0.231$;
this diagnostic adjustment does not substitute for an adequately resolved behavioural measurement.
...
$[-0.000298, +0.002390]$, spans both directions, indicating no detectable
...
cost. Beyond the separate contrasts against baseline, a direct paired comparison
between \textsc{xsa} and \textsc{random} across the eight seeds confirms that
self-directed removal outperforms the matched arbitrary control directly
(mean paired difference $-0.003980$ nats, $t = -4.86$, $p = 0.0018$).
...
\paragraph{Training and scale.} Check~2 evaluates learned intervention in a
50.9M parameter model trained for $4.0\times10^{8}$ tokens per run. It does
not determine effect signs or magnitudes at larger scales, such as the
0.7B--2.7B regimes evaluated in prior work. Furthermore, the frozen and trained
settings differ in gate constraints and first-token handling, so their outcomes
reflect related but non-identical interventions.

\paragraph{Statistic families.} Both evaluated structural statistics rely on
cosine similarities. Alternative formulations based on attention probabilities
or projection norms may exhibit different empirical associations with head
interventions.

\paragraph{Reliability coverage and correlation scope.} Intervention reliability
was evaluated on three models in the main study, and extended to Pythia-1.4B ($r_\Delta = 0.67$)
and Pythia-2.8B ($0.63$; Appendix~\ref{app:extended_reliability}), confirming that single-head
effects become resolvable at scale. The remaining seven ladder models lack intervention measurements.
Disattenuation applies half-sample reliabilities to pooled correlations, meaning adjusted values
serve as diagnostic bounds alongside raw values rather than corrected ground truth. Rank correlations
describe within-sample monotonicity rather than out-of-sample prediction.

\paragraph{Control scope.} While \textsc{random} and \textsc{xsa} share a
rank-one parametric form, their projected norms and converged gate strengths
diverge. The trained comparison does not isolate projection direction from
magnitude. Additionally, reconstruction gates verify captured tensor layouts
and indexing, but do not prove that a value-aligned direction carries solely
...
Learned self-directed removal lowers validation loss by $0.0029$ nats in our
eight-seed factorial ($p = 0.0023$) and outperforms a matched arbitrary rank-one
control directly by $0.0040$ nats ($p = 0.0018$). The arbitrary-direction control
does not differ from baseline ($+0.0011$ nats, $p = 0.197$). These results
demonstrate that the training benefit at 50.9M scale is specific to the
direction removed.

However, a within-sequence null shows that background representation geometry
accounts for $48\%$ to $63\%$ of raw self-value alignment across nine models
through 6.9B parameters, and grouped-query attention reverses the alignment
across key--value groups. Furthermore, the intervention effect is reliably
resolvable in only one of three evaluated models. Large internal alignments alone
do not demonstrate representational redundancy. Modifying representations based
on internal statistics requires establishing measurement resolvability,
empirical null comparisons, and matched directional controls.
...
as such. An anonymised reproduction archive containing all code, configuration
records, raw per-head logs, and validation scripts accompanies this submission
in the supplementary material, and will be released under an open-source license
upon publication.
...
\section{Extended Reliability at Scale}
\label{app:extended_reliability}

To determine whether the attenuation observed in Pythia-160M ($r_\Delta = 0.419$) and Pythia-410M ($0.531$) is an artifact of smaller parameter counts or persists at scale, we extended Check~0 split-half reliability and Check~0--A2 correlations to \texttt{EleutherAI/pythia-1.4b} ($24$ layers, $16$ query heads, $n = 384$) and \texttt{EleutherAI/pythia-2.8b} ($32$ layers, $32$ query heads, $n = 1024$). Both models were evaluated on the identical Check~0 protocol ($64$ documents at a fixed sequence length of $128$ tokens, $32$ documents per half, evaluated in \texttt{bfloat16} on an NVIDIA RTX 6000 Ada Generation GPU).

Both larger models pass the $r_\Delta \ge 0.60$ threshold and are classified as \textsc{reliable} (Table~\ref{tab:extended_reliability}). This establishes that single-head loss perturbations become reliably resolvable with parameter scale across the Pythia family ($r_\Delta$ rises from $0.419$ to $0.531$, $0.666$, and $0.632$).
...
\toprule
Model & Statistic & $r_\Delta$ & $r_{\text{stat}}$ & $\rho_{\max}$ & $\rho_{\text{raw}}$ & $\rho_{\text{disatt}}$ & Verdict \\
\midrule
\multirow{2}{*}{Pythia-1.4B}
  & self-cosine & \multirow{2}{*}{0.666} & 0.982 & 0.809 & $+0.187$ & $+0.232$ & \multirow{2}{*}{reliable} \\
  & excess      &                        & 0.988 & 0.811 & $+0.249$ & $+0.307$ & \\
\midrule
\multirow{2}{*}{Pythia-2.8B}
  & self-cosine & \multirow{2}{*}{0.632} & 0.967 & 0.782 & $+0.003$ & $+0.003$ & \multirow{2}{*}{reliable} \\
  & excess      &                        & 0.982 & 0.788 & $-0.124$ & $-0.158$ & \\
\bottomrule
\end{tabular}
\caption{Check~0 split-half reliability and Check~0--A2 rank correlations for extended Pythia models ($32$ documents per half, $128$ tokens). $A@V$ reconstruction Frobenius error was $1.66\times 10^{-3}$ on Pythia-1.4B and $1.66\times 10^{-3}$ on Pythia-2.8B. As in Table~\ref{tab:a2}, disattenuation uses each statistic's own split-half reliability.}
\label{tab:extended_reliability}
\end{table}

The association between representation statistics and intervention outcomes shows strong scale sensitivity. In Pythia-1.4B, the null-corrected excess cosine ($\rho_{\text{disatt}} = +0.307$) outperforms raw self-cosine ($+0.232$) in predicting intervention effect, mirroring Pythia-410M. In Pythia-2.8B, raw self-cosine completely decouples from intervention effect ($\rho_{\text{raw}} = +0.003, \rho_{\text{disatt}} = +0.003$), despite high measurement reliability ($r_\Delta = 0.632$). This confirms that high representation-level cosine alignment does not reliably index intervention sensitivity.
